...
import {hooks as colocatedHooks} from "phoenix-colocated/flashwars"
  ...
  , GuestName: {
    mounted() {
      ...
      this.el.addEventListener("submit", (e) => {
        ...
        try { if (val.length > 0) localStorage.setItem("fw_duel_name", val) } catch (_e) {}
      })

      this.handleEvent("store_guest_name", ({name}) => {
        try { if (name && name.trim().length > 0) localStorage.setItem("fw_duel_name", name.trim()) } catch (_e) {}
      })
    }
  }
}
